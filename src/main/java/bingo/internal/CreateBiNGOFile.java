...
public class CreateBiNGOFile {
	 ...
	 */
	public CreateBiNGOFile(Map testMap, Map correctionMap, Map mapSmallX, Map mapSmallN,
			Map mapBigX, Map mapBigN, String alphaString, Annotation annotation, Set deleteCodes,
			...
			String catString, Set selectedCanonicalNameVector, Set noClassificationsSet) {
	...
	public void makeFile() {
		...
		try {
			...
			if (testString.equals(NONE)) {
				output.write("GO-ID" + "\t" + "x" + "\t" + "n" + "\t" + "X" + "\t" + "N" + "\t" + "Description" + "\t"
						+ "Genes in test set" + "\n");
			} else if (correctionString.equals(NONE)) {
				output.write("GO-ID" + "\t" + "p-value" + "\t" + "x" + "\t" + "n" + "\t" + "X" + "\t" + "N" + "\t"
						+ "Description" + "\t" + "Genes in test set" + "\n");
			} else {
				output.write("GO-ID" + "\t" + "p-value" + "\t" + "corr p-value" + "\t" + "x" + "\t" + "n" + "\t" + "X"
			...
			for (int i = 0; (i < ordenedKeySet.length) && (ok == true); i++) {
				...
				if (testString.equals(NONE)) {
					output.write(termID + "\t" + smallX + "\t" + smallN + "\t" + bigX + "\t" + bigN + "\t"
							+ description + "\t");
					...
					if (catString.equals(CATEGORY_BEFORE_CORRECTION)) {
						if ((new BigDecimal(testMap.get(new Integer(ordenedKeySet[i])).toString()))
							...
							output.write(termID + "\t" + pvalue + "\t" + smallX + "\t" + smallN + "\t" + bigX + "\t"
									+ bigN + "\t" + description + "\t");
							if (annotatedGenes.containsKey(termID)) {
								Iterator k = ((HashSet) annotatedGenes.get(termID)).iterator();
								while (k.hasNext()) {
					...
					if (catString.equals(CATEGORY_CORRECTION)) {
						if ((new BigDecimal(correctionMap.get(ordenedKeySet[i]).toString())).compareTo(new BigDecimal(
								alphaString)) < 0) {
							output.write(termID + "\t" + pvalue + "\t" + correctedPvalue + "\t" + smallX + "\t"
						...
						output.write(termID + "\t" + pvalue + "\t" + correctedPvalue + "\t" + smallX + "\t" + smallN
